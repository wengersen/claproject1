...
export interface FoodRecord {
  productId: string      // 对应 CatFood.id
  productName: string    // 冗余存储，防止产品数据变更时丢失
  brand: string          // 冗余存储品牌名
  setAt: string          // ISO 时间戳，设为主食的时间
  removedAt?: string     // ISO 时间戳，切换掉的时间（undefined = 当前在喂）
}
...
export interface Pet {
  ...
  gender: 'male' | 'female'
  neutered: boolean
  birthday: string      // 生日（YYYY-MM-DD），替换原 ageMonths
  weightKg: number      // 基准体重
  resultId?: string     // 从推荐结果创建时存储来源 resultId，用于"推荐来源"关联
  currentMainFood?: FoodRecord   // 当前主食（未设置时 undefined）
  foodHistory: FoodRecord[]      // 历史换粮记录（按 removedAt 降序）
  createdAt: string
  updatedAt: string
}
