...
/** 从生日（YYYY-MM-DD）计算当前月龄（向下取整，最小为 0） */
export function calcAgeMonthsFromBirthday(birthday: string): number {
...
/** 从生日格式化为展示字符串，如"1岁3个月"、"8个月" */
export function formatAgeFromBirthday(birthday: string): string {
  const m = calcAgeMonthsFromBirthday(birthday)
  if (m < 1) return '不足1个月'
  if (m < 12) return `${m}个月`
