...
import type { Pet, PetHealthLog, HealthAssessment, FoodRecord } from '@/types/pet'
...
export function getPets(username: string): Pet[] {
  ...
  try {
    ...
    if (!raw) return []
    const raw_pets = JSON.parse(raw) as Array<Pet & { ageMonths?: number }>
    const pets = raw_pets.map((p): Pet => {
      // 旧数据兼容：ageMonths → 估算 birthday
      if (!p.birthday && p.ageMonths) {
        const d = new Date()
        d.setMonth(d.getMonth() - p.ageMonths)
        return { ...p, birthday: d.toISOString().slice(0, 10), foodHistory: p.foodHistory ?? [] }
      }
      return { ...p, foodHistory: p.foodHistory ?? [] }
    })
    return [...pets].sort(
...
/**
 * 设置宠物当前主食
 * - 若已有 currentMainFood，将其加入 foodHistory（补充 removedAt）
 * - 写入新 currentMainFood
 */
export function setCurrentMainFood(
  username: string,
  petId: string,
  food: Omit<FoodRecord, 'setAt' | 'removedAt'>
): void {
  if (typeof window === 'undefined') return
  try {
    ...
    const history = [...(pet.foodHistory ?? [])]
    if (pet.currentMainFood) {
      history.unshift({ ...pet.currentMainFood, removedAt: now })
    }
    pets[idx] = {
      ...pet,
      currentMainFood: { ...food, setAt: now },
      foodHistory: history,
      updatedAt: now,
    }
    localStorage.setItem(petsKey(username), JSON.stringify(pets))
  } catch { /* ignore */ }
}
