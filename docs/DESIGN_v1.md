...
## 十、UI 响应式优化记录 (v1.2 新增)

针对移动端适配问题的专项优化：

### 1. Bento Grid 流程卡片优化 (Item E)
- **问题**：`flex` 布局在小屏下导致文字折行拥挤。
- **优化**：
  - Mobile (< 640px): `grid grid-cols-1 gap-6` 垂直排列。
  - Tablet+ (≥ 640px): `grid-cols-3` 横向排列。
  - 增加步骤序号的视觉显著性，确保垂直流的阅读顺序。

### 2. ProductCard 移动端布局
- **问题**：信息过载，价格与标题挤压。
- **优化**：
  - Mobile: 改为垂直堆叠布局 (`flex-col`)。
  - 价格移至标题下方，与"月均消费"同行。
  - 隐藏非核心标签或简化"喂食建议"展示。
  - 确保点击区域高度 ≥ 44px。

### 3. 字体与间距一致性
- **标题**：使用动态视口字号或断点字号 `text-[22px] md:text-[28px]`。
- **Modal**：移动端 Padding 缩减至 `p-6`，最大化内容显示区域。
- **Container**：统一 `px-4 md:px-8 lg:px-16`。
...
*v1.1 — 2026-03-01 新增认证弹窗 + AuthNav + Step 0 多宠物卡片 + Profile/宠物档案页/LogEntryModal 设计规范*
*v1.2 — 2026-03-05 UI 响应式排查与优化（针对 Bento Grid, ProductCard, Spacing）*
