...
| **v1.2** | **2026-03-05** | **智能营养引擎：冲突纠偏系统 + 加载体验优化 + 保存 Bug 修复** |
| **v1.2.1** | **2026-03-05** | **UI 体验升级：移动端适配 (Bento/Card)、字体层级优化** |
...
### 3.1 功能优先级矩阵（含 v1.2 更新）

| 功能模块 | 优先级 | 状态 | 备注 |
|---------|--------|------|------|
| 猫咪信息录入 | P0 | ✅ 已上线 | 品种/生日/体重/性别/绝育 |
| 健康需求选择 | P0 | ✅ 已上线 | 10种健康标签 |
| AI 推荐引擎（SSE） | P0 | ✅ 已上线 | 干粮6款 + 湿粮3款，流式生成 |
| 推荐结果展示 | P0 | ✅ 已上线 | 产品卡片 + 喂食指南 |
| **需求冲突纠偏** | **P0** | **✅ v1.2** | **4条规则，自动调整健康标签** |
| 用户注册/登录 | P1 | ✅ 已上线 | 用户名+密码，JWT 认证 |
| 宠物档案保存 | P1 | ✅ 已上线 | 登录后保存，v1.2 修复保存失败 Bug |
| 多猫档案管理 | P1 | ✅ 已上线 | 多宠物选择界面 |
| 产品对比功能 | P1 | ✅ 已上线 | 3维度对比 |
| 购买链接 | P2 | 📋 预留 | 上线后接入联盟 |
| 健康事件记录 | P2 | ❌ 规划中 | v1.5 规划 |
| 用户评价/反馈 | P2 | ❌ 规划中 | v2 规划 |
...
#### 业务背景
用户在填写健康需求时，可能因认知偏差选择与猫咪实际健康状态矛盾的标签（如：超重猫选择"增重/营养"）。直接按用户选择推荐可能造成健康风险，需要系统自动识别并纠偏。

#### 技术规格

**品种体重判断**（`lib/breedWeights.ts`）

| 参数 | 说明 |
|------|------|
| 覆盖品种 | 22 个（含"混合/土猫"兜底） |
| 幼猫系数 | 月龄 0~12 月线性插值，系数 0.3 → 1.0 |
| 判定阈值 | 超出范围 20% 以上为"超重"，低于 20% 以下为"偏轻" |

**纠偏规则**（`lib/conflictDetector.ts`）

| 规则ID | 触发条件 | 原标签 | 纠偏为 | 优先级 |
|--------|---------|--------|--------|--------|
| R01 | 实测体重 > 标准上限 × 1.2 | weight_gain（增重） | weight（体重管理） | 高 |
| R02 | 实测体重 < 标准下限 × 0.8 | weight（体重管理） | weight_gain（增重） | 高 |
| R03 | 年龄 < 12 个月 | senior（老年护理） | 移除 | 中 |
| R04 | 年龄 > 84 个月（7岁） | kitten（幼猫发育） | 移除 | 中 |

**兜底逻辑**：若所有标签均被移除，自动补充 `balanced`（日常均衡）标签。

**SSE 事件**：`conflicts`（在 LLM 调用前发送）
...
// ConflictItem
{
  originalTag: HealthTag      // 原始标签
  correctedTag?: HealthTag    // 纠偏后标签（无则表示已移除）
  message: string             // 温和解释型顾问文案
}
```
...
**生成中（临时卡）**
- 位置：LoadingState 组件下方
- 样式：蓝色边框卡片（`bg-blue-50 border-blue-200`）
- 图标：🔍
- 标题：「营养方案已为您调整」
- 内容：冲突说明文案 + 标签对比（原标签划线 → 纠偏标签高亮）
...
| 序号 | 文案 |
|------|------|
| 1 | 正在解析 {name} 的健康档案与营养需求... |
| 2 | 从数百款宠物食品数据库中筛选候选方案... |
| 3 | 专业营养师模型分析蛋白质 · 脂肪 · 矿物质配比... |
| 4 | 交叉验证 12 项核心健康指标，排除不适配成分... |
| 5 | 根据 {name} 的体型与年龄阶段量身定制推荐方案... |
| 6 | 对候选产品进行品质可信度综合评分... |

- 轮播间隔：2800ms
- 淡入淡出：300ms
- 进度条：独立由 SSE `progress` 事件控制，不与文案轮播耦合
- 副标题：「营养顾问正在为您的猫咪深度定制，请稍候 🐾」

### 3A.3 产品推荐理由数据迁移

| 维度 | 迁移前 | 迁移后 |
|------|--------|--------|
| reason 字段来源 | LLM 动态生成 | `data/catfoods.json` 静态存储 |
| LLM 输出内容 | productId + reason + personalNote + highlights + feedingGuide | productId + personalNote + highlights + feedingGuide |
| 预估 token 节省 | — | ~60% |
| 前端处理 | 直接使用 LLM 输出 | `hydrateSlimResult()` 合并静态 DB |
